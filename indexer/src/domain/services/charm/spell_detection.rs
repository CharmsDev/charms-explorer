...
impl<'a> SpellDetector<'a> {
    ...
    pub async fn detect_from_hex(
        ...
        let (normalized_spell, spell_json) = match NativeCharmParser::extract_and_verify_charm(raw_tx_hex, false) {
            Ok(spell) => {
                let spell_json = serde_json::to_value(&spell)
        ...
        if let Err(e) = self.spell_repository.save_spell(&spell).await {
            ...
            return Err(CharmError::ProcessingError(format!("Failed to save spell: {}", e)));
        }
        ...
        for (index, asset_info) in asset_infos.iter().enumerate() {
            ...
            if let Some(ref queue_service) = self.charm_queue_service {
                ...
                if let Err(e) = queue_service.save_charm_data(
                    &charm,
                    tx_pos as i64,
        ...
        crate::utils::logging::log_info(&format!(
            ...
            network, block_height, charms.len(), txid
        ));
