...
export const API_BASE_URL =
  process.env.NEXT_PUBLIC_CHARMS_API_URL || "http://localhost:8000/v1";

// API Endpoints
export const ENDPOINTS = {
  CHARMS: `${API_BASE_URL}/charms`,
  CHARM_BY_CHARMID: (charmid) =>
    `${API_BASE_URL}/charms/by-charmid/${encodeURIComponent(charmid)}`,
  CHARM_BY_TXID: (txid) => `${API_BASE_URL}/charms/${txid}`,
  CHARMS_BY_ADDRESS: (address) =>
  ...
  ASSET_HOLDERS: (appId) =>
    `${API_BASE_URL}/assets/${encodeURIComponent(appId)}/holders`, // [RJJ-STATS-HOLDERS]
  REFERENCE_NFT: (hash) =>
    `${API_BASE_URL}/assets/reference-nft/${encodeURIComponent(hash)}`, // [RJJ-REF-NFT]
  ...
  DEX_ORDERS_BY_MAKER: (maker) =>
    `${API_BASE_URL}/dex/orders/by-maker/${encodeURIComponent(maker)}`,
  DEX_ORDER_BY_ID: (orderId) =>
    `${API_BASE_URL}/dex/orders/${encodeURIComponent(orderId)}`,
